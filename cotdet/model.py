...
class CoTDet(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # Loss parameters:
        deep_supervision = cfg.MODEL.CoTDet.DEEP_SUPERVISION
        no_object_weight = cfg.MODEL.CoTDet.NO_OBJECT_WEIGHT

        # loss weights
        class_weight = cfg.MODEL.CoTDet.CLASS_WEIGHT
        cost_class_weight = cfg.MODEL.CoTDet.COST_CLASS_WEIGHT
        cost_dice_weight = cfg.MODEL.CoTDet.COST_DICE_WEIGHT
        dice_weight = cfg.MODEL.CoTDet.DICE_WEIGHT  #
        cost_mask_weight = cfg.MODEL.CoTDet.COST_MASK_WEIGHT  #
        mask_weight = cfg.MODEL.CoTDet.MASK_WEIGHT
        cost_box_weight = cfg.MODEL.CoTDet.COST_BOX_WEIGHT
        box_weight = cfg.MODEL.CoTDet.BOX_WEIGHT  #
        cost_giou_weight = cfg.MODEL.CoTDet.COST_GIOU_WEIGHT
        giou_weight = cfg.MODEL.CoTDet.GIOU_WEIGHT  #
        # building matcher
        matcher = HungarianMatcher(
            ...
            num_points=cfg.MODEL.CoTDet.TRAIN_NUM_POINTS,
        )
        ...
        if dn == "standard":
            ...
            dn_losses=[]
        if deep_supervision:
            dec_layers = cfg.MODEL.CoTDet.DEC_LAYERS
            aux_weight_dict = {}
            for i in range(dec_layers):
                aux_weight_dict.update({k + f"_{i}": v for k, v in weight_dict.items()})
            weight_dict.update(aux_weight_dict)
        if cfg.MODEL.CoTDet.BOX_LOSS:
        ...
        criterion = SetCriterion(
            ...
            losses=losses,
            num_points=cfg.MODEL.CoTDet.TRAIN_NUM_POINTS,
            oversample_ratio=cfg.MODEL.CoTDet.OVERSAMPLE_RATIO,
            importance_sample_ratio=cfg.MODEL.CoTDet.IMPORTANCE_SAMPLE_RATIO,
            dn=cfg.MODEL.CoTDet.DN,
            dn_losses=dn_losses,
            panoptic_on=(cfg.MODEL.CoTDet.TEST.PANOPTIC_ON or cfg.MODEL.CoTDet.TEST.SEMANTIC_ON) and ~cfg.MODEL.CoTDet.PANO_BOX_LOSS,
            semantic_ce_loss=cfg.MODEL.CoTDet.TEST.SEMANTIC_ON and cfg.MODEL.CoTDet.SEMANTIC_CE_LOSS and ~cfg.MODEL.CoTDet.TEST.PANOPTIC_ON,
        )
        ...
        return {
            ...
            "criterion": criterion,
            "num_queries": cfg.MODEL.CoTDet.NUM_OBJECT_QUERIES,
            "object_mask_threshold": cfg.MODEL.CoTDet.TEST.OBJECT_MASK_THRESHOLD,
            "overlap_threshold": cfg.MODEL.CoTDet.TEST.OVERLAP_THRESHOLD,
            "metadata": MetadataCatalog.get(cfg.DATASETS.TRAIN[0]),
            "size_divisibility": cfg.MODEL.CoTDet.SIZE_DIVISIBILITY,
            "sem_seg_postprocess_before_inference": (
                cfg.MODEL.CoTDet.TEST.SEM_SEG_POSTPROCESSING_BEFORE_INFERENCE
                or cfg.MODEL.CoTDet.TEST.PANOPTIC_ON
                or cfg.MODEL.CoTDet.TEST.INSTANCE_ON
            ),
            "pixel_mean": cfg.MODEL.PIXEL_MEAN,
            "pixel_std": cfg.MODEL.PIXEL_STD,
            # inference
            "semantic_on": cfg.MODEL.CoTDet.TEST.SEMANTIC_ON,
            "instance_on": cfg.MODEL.CoTDet.TEST.INSTANCE_ON,
            "panoptic_on": cfg.MODEL.CoTDet.TEST.PANOPTIC_ON,
            "test_topk_per_image": cfg.TEST.DETECTIONS_PER_IMAGE,
            "data_loader": cfg.INPUT.DATASET_MAPPER_NAME,
            "focus_on_box": cfg.MODEL.CoTDet.TEST.TEST_FOUCUS_ON_BOX,
            "transform_eval": cfg.MODEL.CoTDet.TEST.PANO_TRANSFORM_EVAL,
            "pano_temp": cfg.MODEL.CoTDet.TEST.PANO_TEMPERATURE,
            "semantic_ce_loss": cfg.MODEL.CoTDet.TEST.SEMANTIC_ON and cfg.MODEL.CoTDet.SEMANTIC_CE_LOSS and ~cfg.MODEL.CoTDet.TEST.PANOPTIC_ON
        }
    ...
    def forward(self, batched_inputs):
        ...
        task_ids = [torch.tensor(x["task_id"], device=self.device, dtype=torch.int64) for x in batched_inputs]
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                ...
                targets = None
            outputs,mask_dict = self.sem_seg_head(features, task_ids=task_ids ,targets=targets)
            ...
            outputs, _ = self.sem_seg_head(features, task_ids=task_ids)
    ...
    def instance_inference(self, mask_cls, mask_pred, mask_box_result):
        ...
        #############################################################
        # labels_per_image = (labels_per_image < 15)*labels_per_image
        #############################################################
